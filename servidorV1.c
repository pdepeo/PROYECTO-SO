	...
	for (;;){
		...
		while(terminar==0){
			...
			if(codigo==0) 	
				...
				strcat(consulta, "' AND Jugador.nombre = Relacion.nombreJugador AND Relacion.IdPartida = (SELECT Partida.Id FROM Jugador, Partida, Relacion WHERE Jugador.nombre = '");
				strcat(consulta, nombre2);
				strcat(consulta, "' AND Jugador.nombre = Relacion.nombreJugador AND Relacion.IdPartida = Partida.Id)");
				printf("%s", consulta);
				err=mysql_query (conn, consulta);
				if (err!=0) {
					printf ("Error al consultar datos de la base %u %s\n",
